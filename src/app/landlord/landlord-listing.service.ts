...
export class LandlordListingService {
  ...
  getAll(): void {
    this.http
      ...
      .subscribe({
        next: (listings) =>
          this.getAll$.set(
            State.Builder<Array<CardListing>>().forSuccess(listings)
          ),
        error: (err) =>
          this.create$.set(State.Builder<CreatedListing>().forError(err)),
      });
  ...
  delete(publicId: string): void {
    ...
    this.http
      .delete<string>(`${environment.API_URL}/landlord-listing/delete`, {
        params,
      })
      .subscribe({
        next: (publicId) =>
          this.delete$.set(State.Builder<string>().forSuccess(publicId)),
        error: (err) =>
          this.create$.set(State.Builder<CreatedListing>().forError(err)),
      });
  }
